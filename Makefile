...
# ---------------------------------------------------------------------------
# Repo-owned, and the two things this Makefile carried before v1.4.0 retired the synced
# make layer.
#
# `MKDOCS_EXTRA_PACKAGES = --with 'mkdocstrings[python]'` is *not* one of them: rhiza-task
# 0.3.1 makes `mkdocstrings[python]` the default of `mkdocs_extra_packages`, so the override
# is now redundant rather than lost. Restore it as `mkdocs-extra-packages` in
# `[tool.rhiza-task]` if that default ever changes.
#
# `test-layout` is. basanos groups tests by behaviour rather than mirroring the source tree
# 1:1, so a bespoke checker enforces the two properties that matter (every public module is
# tested; every test traces back to code) with the intentional behaviour-grouping recorded
# in `[tool.check_test_layout]`.
#
# It used to chain on with a double-colon `test:: test-layout`, appending to the `test`
# target `.rhiza/make.d/test.mk` defined. There is no such target now -- only the catch-all
# `%:` above -- so the chain becomes an ordinary explicit rule, which beats a pattern rule
# and therefore still owns `make test`.
#
# **This no longer gates CI.** `rhiza_ci.yml@v1.4.2` runs `uvx rhiza-task test` directly
# instead of `make test` (jebel-quant/rhiza#1546), so the drift this checker catches now
# fails only at review or locally. Restoring the gate means giving the check a home CI does
# reach -- a `tests/test_test_layout.py` collected by the suite, or a local pre-commit hook.
#
# stdlib-only and run with bare `python3`: the checker imports nothing but `sys` and
# `pathlib`, so it needs neither the project environment nor uv.
.PHONY: test test-layout
test-layout: ## check public modules are tested and tests trace back to code
	@echo "[INFO] Checking test-layout parity"
