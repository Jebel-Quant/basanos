...
\hypersetup{
  ...
  pdftitle    = {Correlation-Aware Signal Assessment for Systematic Strategies},
...
% ─── Title ───────────────────────────────────────────────────────────────────
\title{%
  \textbf{Correlation-Aware Signal Assessment\\
  for Systematic Strategies}\\[0.5em]
  \large A Companion Paper
}
\author{Thomas Schmelzer \\ \small\texttt{thomas@jqr.ae}}
\date{\today}
...
signal assessment problem.  Section~\ref{sec:approach} develops the
mathematical pipeline---volatility adjustment, EWMA correlation estimation,
shrinkage, and the normalised linear solve---and closes with a shrinkage sweep
that quantifies what correlation adjustment adds over a proportional baseline.
Section~\ref{sec:factor_models} presents factor risk models as an alternative
covariance parameterisation, including latent-factor extraction via SVD,
efficient inversion via the Woodbury identity, and a sliding-window estimator.
Section~\ref{sec:analytics} documents optimizer-level diagnostics and
portfolio-level analytics.  Section~\ref{sec:usage} illustrates the complete
workflow with a code example.  Section~\ref{sec:compute} summarises
...
We now make the interaction between signal direction and correlation structure
precise.  The simplest sizing rule is \emph{proportional}: set
\begin{equation}
...
\subsection{Shrinkage as a Sensitivity Analysis}
...
The library exposes two layers of analytics.  The first---optimizer-level
diagnostics---is accessed through \texttt{BasanosEngine} and introspects the
numerical behaviour of the solve at each timestamp: condition number, effective
rank, solver residual, signal utilisation, and leverage.  The second---portfolio-level
analytics---is accessed through the \texttt{Portfolio} object returned by
\texttt{engine.portfolio} and measures the risk-adjusted performance of the
resulting positions.
