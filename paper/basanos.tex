...
solve.  Section~\ref{sec:analytics} documents the analytical tools available in
the library, distinguishing optimizer-level diagnostics from portfolio-level
performance analytics.  Section~\ref{sec:usage} describes the development
infrastructure and illustrates usage with code examples.
...
\begin{enumerate}
  ...
  \item \textbf{Correlation estimation} --- one of two paths depending on
    configuration:
    \begin{itemize}
      \item \emph{EWMA path} (default) --- compute the time-varying EWMA
        correlation matrix and regularise it toward the identity
        (Sections~\ref{sec:corr}--\ref{sec:shrinkage}).
      \item \emph{Factor-model path} (when \texttt{cfg.n} and \texttt{cfg.k}
        are set) --- derive a low-rank correlation estimate from the dominant
        principal directions of a rolling return window
        (Section~\ref{sec:factor}).
    \end{itemize}
  \item \textbf{Normalised linear solve} --- solve $\tilde{\C}_t\,\x_t =
    \muvec_t$ for the risk position vector, then scale by realised P\&L
    variance and divide by per-asset volatility to obtain cash positions
...
\begin{equation}
  \hat{C}_{ij,t} = \frac{E_{ij,t} - E_{i,t}\,E_{j,t}}
                        {\sqrt{(E_{i^2,t} - E_{i,t}^2)
                               (E_{j^2,t} - E_{j,t}^2)}}.
\end{equation}
The full set of five running moments ($E_i$, $E_j$, $E_{i^2}$, $E_{j^2}$,
$E_{ij}$) is computed in parallel across all $n^2$ pairs using
...
contributes all $n^2$ cross-moment estimates simultaneously with a single
pass over the data.  The correlation matrix $\hat{\C}_t$ is then obtained
...
\subsection{Factor-Model Covariance (Alternative to EWMA)}
\label{sec:factor}

The EWMA path stores a full $m \times m$ correlation matrix for every
timestamp, requiring $O(T m^2)$ memory.  When the asset universe is large or
the rolling-window approach is preferred, Basanos offers an alternative
covariance source driven by a \textbf{rolling SVD factor model}, activated by
setting \texttt{cfg.n} (lookback rows) and \texttt{cfg.k} (number of factors).

\subsubsection{Rolling Returns Window}

At each timestamp $t$, assemble the $n$ most recent rows of vol-adjusted
returns into the window matrix
\begin{equation}
  \label{eq:factor_window}
  \Rmat_t \in \R^{n \times m},
\end{equation}
where $n = \texttt{cfg.n}$ and $m$ is the number of assets.  The window slides
forward by one row each period: the oldest row is dropped and the current row
$\tilde{\mathbf{r}}_t$ is appended.  Missing values (NaN) are filled with zero
--- the neutral, no-information fill that keeps every window finite without
biasing the principal directions.

\subsubsection{Factor Extraction via Truncated SVD}

Compute the economy-size singular value decomposition
\begin{equation}
  \label{eq:svd}
  \Rmat_t = \mathbf{U}_t\,\mathbf{S}_t\,\mathbf{V}_t^\top,
  \quad \mathbf{U}_t \in \R^{n \times \min(n,m)},\;
        \mathbf{S}_t \in \R^{\min(n,m) \times \min(n,m)},\;
        \mathbf{V}_t \in \R^{m \times \min(n,m)}.
\end{equation}
Retain the first $k = \texttt{cfg.k}$ left singular vectors as the
\emph{factor matrix}:
\begin{equation}
  \label{eq:factor_matrix}
  \mathbf{F}_t = \mathbf{U}_t^{(k)} \in \R^{n \times k},
\end{equation}
where $\mathbf{U}_t^{(k)}$ denotes the first $k$ columns of $\mathbf{U}_t$.
The columns of $\mathbf{F}_t$ are the $k$ principal directions of the return
history that capture the most variance over the lookback window.  They are
orthonormal by construction: $\mathbf{F}_t^\top\mathbf{F}_t = \I_k$.

\subsubsection{Asset Projection}

Project all $m$ asset columns of $\Rmat_t$ onto the factor space:
\begin{equation}
  \label{eq:projection}
  \W_t = \mathbf{F}_t^\top \Rmat_t \in \R^{k \times m}.
\end{equation}
Each column of $\W_t$ is the factor-exposure vector for one asset; each row
is one factor's loadings across the universe.

\subsubsection{Column Normalisation and the Woodbury Solve}
\label{sec:factor_woodbury}

The columns of $\W_t$ encode each asset's exposure to the $k$ latent factors.
Define the \emph{column-normalised} matrix
\begin{equation}
  \label{eq:factor_norm}
  \hat{W}_{t,i} = \frac{w_{t,i}}{\|w_{t,i}\|_2},
  \quad i = 1, \ldots, m,
\end{equation}
where $w_{t,i} \in \R^k$ is the $i$-th column of $\W_t$.  By construction
\begin{equation}
  \label{eq:factor_corr_identity}
  \hat{\W}_t^\top \hat{\W}_t = \hat{\C}_t \in \R^{m \times m},
\end{equation}
is the sample correlation matrix.  \textbf{This product is never formed.}
Instead, it enters only through the shrunk linear system

\begin{equation}
  \label{eq:factor_system}
  M_t\,\x_t = \muvec_t, \quad
  M_t = \lambda\,\hat{\W}_t^\top\hat{\W}_t + (1-\lambda)\,\I_m,
\end{equation}

whose solution requires only a $k \times k$ computation via the
...
Setting $A=(1-\lambda)\I_m$, $U=\hat{\W}_t^\top$, $C=\lambda\I_k$,
$V=\hat{\W}_t$ yields

\begin{equation}
  \label{eq:factor_gram}
  G_t = \I_k + \tfrac{\lambda}{1-\lambda}\,\hat{\W}_t\hat{\W}_t^\top
  \quad\in\R^{k \times k},
\end{equation}

\begin{equation}
  \label{eq:factor_woodbury_result}
  M_t^{-1}\muvec_t = \frac{1}{1-\lambda}
  \Bigl[\muvec_t
   - \tfrac{\lambda}{1-\lambda}\,\hat{\W}_t^\top
     G_t^{-1}\!\bigl(\hat{\W}_t\muvec_t\bigr)\Bigr].
\end{equation}

The only matrix ever formed is the \emph{$k \times k$ Gram matrix} $G_t$; the
remaining operations are two matrix--vector products of cost $O(km)$ each,
plus one $k \times k$ Cholesky solve of cost $O(k^3)$.  Total per-step cost:
$O(km + k^3)$ versus $O(m^3)$ for a direct solve on the $m \times m$ system.

\paragraph{Applying shrinkage.}
$G_t$ already incorporates the shrinkage weight $\lambda$; no separate
shrinkage step is required.  Setting $\lambda=0$ (full shrinkage toward the
identity) gives $G_t = \I_k$, $M_t^{-1}=\I_m$, and $\x_t \propto \muvec_t$
--- the signal-proportional sizing baseline.  The normalised solution
$\x_t = M_t^{-1}\muvec_t / \|M_t^{-1}\muvec_t\|_{M_t}$ then passes to the
same profit-variance scaler described in Section~\ref{sec:solve}.

\subsubsection{Memory and Compute Comparison}

\begin{table}[h]
  \centering
  \caption{Memory and per-step compute for the two covariance paths.
           $T$ = total timestamps, $m$ = assets, $n$ = lookback rows,
           $k$ = factors.}
  \label{tab:factor_complexity}
  \begin{tabular}{lll}
    \toprule
    \textbf{Path} & \textbf{Peak memory} & \textbf{Per-step compute} \\
    \midrule
    EWMA (default) & $O(T m^2)$ & $O(m^2)$ via \texttt{lfilter} + $O(m^3)$ solve \\
    Factor model   & $O(n m)$   & $O(nk)$ SVD + $O(km)$ projection + $O(k^3)$ Woodbury \\
    \bottomrule
  \end{tabular}
\end{table}

\noindent
The factor model is advantageous when $m$ is large and $k \ll m$: storage
reduces from $O(T m^2)$ to $O(nm)$, and the per-step SVD truncates at
rank $k$ rather than computing all $m^2$ EWMA pairs.

\begin{remark}
  The factor model is the natural finite-sample analogue of the outer-product
  interpretation in Section~\ref{sec:corr}: whereas the EWMA outer products
  accumulate exponentially over all $T$ rows, the factor model uses only the
  $n$ most recent rows and compresses them into $k$ directions via SVD.  As
  $n \to T$ and $k \to m$ the two estimates converge (modulo the exponential
  weighting).
\end{remark}
...
and denominator scale identically.  This separates position sizing (handled
by the dynamic risk scalar below) from signal direction.

\paragraph{Dynamic risk scaling.}
A running EMA of realised squared portfolio P\&L,
\begin{equation}
  V_t = \beta\,(\hat{\x}_{t-1}^\top \mathbf{r}_t)^2 + (1-\beta)\,V_{t-1},
  \quad V_0 = V_\text{init},
\end{equation}
is maintained with decay factor $\beta$ (\texttt{cfg.profit\_variance\_decay},
default $0.99$).  Positions are further divided by $V_t$:
\begin{equation}
  \label{eq:risk_scaling}
  \tilde{\x}_t = \frac{\hat{\x}_t}{V_t}.
\end{equation}
This adapts risk dynamically: in regimes of high realised P\&L variance the
system reduces exposure, and in calm regimes it scales up.
...
\begin{equation}
  ...
  p_{i,t} = \frac{\tilde{x}_{i,t}}{\sigma_{i,t}} \times \delta,
\end{equation}
...
print(portfolio.stats.sharpe())        # Annualised Sharpe ratio
print(portfolio.stats.max_drawdown())  # Maximum drawdown
...
\begin{table}[h]
  ...
  \begin{tabular}{>{\ttfamily}lp{6cm}l}
    ...
    profit\_variance\_init  & Initial value for P\&L variance EMA. & 1.0 \\
    profit\_variance\_decay & EMA decay factor for realised P\&L variance. & 0.99 \\
    denom\_tol & Zero threshold for normalisation denominator. & $10^{-12}$ \\
    position\_scale & Scaling factor applied before AUM normalisation. & $10^6$ \\
    \bottomrule
  \end{tabular}
\end{table}
