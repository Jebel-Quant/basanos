...
\begin{equation}
  ...
  \mathbf{F}_t = \mathbf{U}_t[:,\,0{:}k] \in \R^{n \times k}.
\end{equation}
...
\subsubsection{Low-Rank Covariance and Correlation}

Form the rank-$k$ sample covariance estimate in the original asset space:
\begin{equation}
  \label{eq:factor_cov}
  \hat{\Sig}_t = \frac{1}{k}\,\W_t^\top \W_t \in \R^{m \times m}.
\end{equation}
Dividing by $k$ normalises the scale so that the diagonal entries are
comparable across different values of $k$.

Normalise entry-wise to a correlation matrix:
\begin{equation}
  \label{eq:factor_corr}
  \hat{C}_{ij,t}
  = \frac{\hat{\Sigma}_{ij,t}}
         {\sqrt{\hat{\Sigma}_{ii,t}\,\hat{\Sigma}_{jj,t}}},
  \quad \hat{C}_{ii,t} = 1.
\end{equation}
This correlation matrix is then shrunk toward the identity exactly as in
Eq.~\eqref{eq:shrinkage}:
\begin{equation}
  \label{eq:factor_shrunk}
  \tilde{\C}_t = \lambda\,\hat{\mathbf{C}}_t + (1-\lambda)\,\I_m,
\end{equation}
and passed to the same normalised linear solver described in
Section~\ref{sec:solve}.
...
\begin{table}[h]
  ...
  \begin{tabular}{lll}
    ...
    \midrule
    EWMA (default) & $O(T m^2)$ & $O(m^2)$ via \texttt{lfilter} \\
    Factor model   & $O(n m)$   & $O(nm)$ window + $O(nk)$ SVD \\
    \bottomrule
  \end{tabular}
\end{table}
